#!/usr/bin/env python3
"""Fit and run the global FKPP forecasting baseline."""
...
def main() -> int:
    ...
    parser.add_argument(
        "--no-write",
        action="store_true",
        help="Fit and evaluate FKPP without writing outputs.",
    )
    ...
    predicted_scaled = model.predict(baseline_scaled, dataset.time_years, rho=fit.rho, alpha=fit.alpha)
    ...
    prediction_rows = build_prediction_rows(dataset, predicted, split, "global_fkpp")
    ...
    if not args.no_write:
        predictions_path = output_dir / outputs.get("fkpp_predictions_table", "fkpp_forecast_predictions.csv")
        metrics_path = output_dir / outputs.get("fkpp_metrics_table", "fkpp_forecast_metrics.csv")
        fit_report_path = output_dir / outputs.get("fkpp_fit_report", "fkpp_fit_report.json")
        aggregate_metrics_path = output_dir / outputs.get("fkpp_metrics_summary", "fkpp_forecast_metrics_summary.csv")
        ...
        print(f"fkpp_fit_report: {fit_report_path}")
