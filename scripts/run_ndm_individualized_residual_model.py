#!/usr/bin/env python3
"""Fit NDM backbone plus individualized residual correction.

NDM provides an exact diffusion solution (exp(-rho*t*L) @ S0), optimising a
single scalar rho on the training set.  The individualized residual layer then
corrects the per-pair, per-region deviation with a ridge regression on physics,
subject, and regional features — identical to the FKPP+IR variant but using the
NDM backbone so the correction captures *non-diffusive* individual dynamics.
"""
...
MODEL_NAME = "ndm_individualized_residual"
...
def main() -> int:
    ...
    result = run_ndm_individualized_residual_model(dataset, split, laplacian, modeling, config)
    ...
    report = {
        "model": MODEL_NAME,
        "purpose": (
            "NDM backbone (exact diffusion exp(-rho*t*L)) plus train-only individualized residual "
            "correction using NDM, subject, regional, and region-bias features."
        ),
        "equation": "S1_hat = NDM(S0, connectome, rho, dt) + gamma * dt * ridge_residual_rate(features)",
    ...
    print("\nNDM + individualized residual comparison:")
    ...
    if not args.no_write:
        write_csv_rows(
            output_dir / outputs.get("ndm_individualized_residual_pair_metrics", "ndm_individualized_residual_pair_metrics.csv"),
            pair_metrics,
        )
        write_csv_rows(
            output_dir / outputs.get("ndm_individualized_residual_metrics_summary", "ndm_individualized_residual_metrics_summary.csv"),
            metrics_summary,
        )
        write_csv_rows(
            output_dir / outputs.get("ndm_individualized_residual_likelihood_metrics", "ndm_individualized_residual_likelihood_metrics.csv"),
            likelihood_metrics,
        )
        write_csv_rows(
            output_dir / outputs.get("ndm_individualized_residual_terms", "ndm_individualized_residual_terms.csv"),
            result["term_rows"],
        )
        write_json(
            output_dir / outputs.get("ndm_individualized_residual_report", "ndm_individualized_residual_report.json"),
            {**report, "likelihood_metrics": likelihood_metrics},
        )
        print("\nWrote NDM+IR outputs.")

    return 0


def run_ndm_individualized_residual_model(
    ...
    ndm_model = NetworkDiffusionModel(laplacian)
    backbone_fit = ndm_model.fit_global_rho(
    ...
    backbone_scaled = ndm_model.predict(baseline_scaled, dataset.time_years, rho=backbone_fit.rho)
    ...
    features = build_individualized_residual_features(
        ...
        include_region_bias=bool(modeling.get("ndm_individualized_residual_region_bias", True)),
        backbone_name="ndm",
    )
    ...
    fit = fit_ridge_residual_model(
        ...
        alphas=tuple(
            float(v) for v in modeling.get("ndm_individualized_residual_ridge_alphas", [0.1, 1, 10, 100, 1000])
        ),
        cv_folds=int(modeling.get("ndm_individualized_residual_cv_folds", 5)),
        max_train_rows=int(modeling.get("ndm_individualized_residual_max_train_rows", 60000)),
        random_seed=random_seed,
    )
    residual_rate = fit.predict_rate(features.values)
    shrinkage_candidates = tuple(
        float(v) for v in modeling.get("ndm_individualized_residual_shrinkage_candidates", [0, 0.25, 0.5, 0.75, 1.0])
    )
    ...
    n_likelihood_parameters = 3 + len(fit.feature_names)  # rho + shrinkage + sigma
    ...
    return {
        ...
        "fit_report": {
            ...
            "backbone_optimizer_success": backbone_fit.optimizer_success,
            "residual_target": "scaled residual rate = (observed_scaled - ndm_scaled) / dt",
...
def build_r2_report(
    ...
    return {
        ...
        "train_scalar_r2_ndm_ir": scalar_r2(dataset.observed[train], predicted[train]),
        "test_scalar_r2_backbone": scalar_r2(dataset.observed[test], backbone_predicted[test]),
        "test_scalar_r2_ndm_ir": scalar_r2(dataset.observed[test], predicted[test]),
        "test_scalar_r2_gain": (
            scalar_r2(dataset.observed[test], predicted[test])
