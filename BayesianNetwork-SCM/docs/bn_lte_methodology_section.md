# Methodology: Bayesian Networks with Latent Time Embedding

## Scope of This Section

This section describes the implemented ADNI-first Bayesian Network with Latent
Time Embedding (BN-LTE) pipeline in `BayesianNetwork-SCM`. The method covers
longitudinal target construction, train-only latent disease-time estimation,
expert-constrained dynamic Bayesian-network structure, spline-varying
structural effects, penalized estimation, held-out forecasting, dynamic edge
summaries, and comparison against mechanistic baselines such as NDM, ESM, and
SIR.
...
The empirical unit is a longitudinal subject-pair. Let `i = 1, ..., N` index
eligible baseline-to-follow-up pairs, and let `u(i)` denote the corresponding
participant. Each pair has a baseline visit time `t_i0`, a follow-up visit time
`t_i1`, and elapsed time
...
in years. Let `X_i(t_i0) in R^p` be the baseline multimodal feature vector,
let `Y_i(t_i0) in R^q` be the baseline value of the target biomarkers or
regions, and let `Y_i(t_i1) in R^q` be the corresponding follow-up value. The
annualized rate target for region or biomarker `j` is

```math
R_{ij}
= \frac{Y_{ij}(t_{i1}) - Y_{ij}(t_{i0})}{\Delta_i},
\qquad
R_i = [R_{i1}, \ldots, R_{iq}]^T .
```

The key latent variable is `Z_i in [0,1]`, an estimated disease-stage coordinate
computed from baseline features only. Low values correspond to earlier
biological disease stage and high values correspond to more advanced stage.
The variable `Z_i` is not a hidden calendar time and is not fitted using the
held-out follow-up targets; it is a train-derived, explainable ordering of
baseline disease burden.

For each target `j`, BN-LTE learns a stage-indexed directed graph
`G(z) = (V, E(z))`, where the node set `V` contains baseline variables and
future rate targets, and `E(z)` contains directed baseline-to-rate dependencies
that may vary with disease stage. The admissible parent set of target `j` at
stage `z` is

```math
\operatorname{pa}_z(j)
= \{l : X_l(t_0) \rightarrow R_j \text{ is active at stage } z\}.
```
...
| Symbol | Meaning |
| --- | --- |
| `X_{il}(t_i0)` | Baseline value of predictor `l` for pair `i`. |
| `Y_{ij}(t_i0)` | Baseline burden of target `j`. |
| `Y_{ij}(t_i1)` | Follow-up burden of target `j`. |
...
| `pa_z(j)` | Parents allowed to directly affect target-rate `R_j` at stage `z`. |
| `a_j(z)` | Stage-specific intercept or population drift term. |
| `c_j(z)` | Stage-specific self-history effect of baseline `Y_j(t_i0)` on its future rate. |
| `b_{jl}(z)` | Stage-specific direct effect of baseline predictor `X_l(t_i0)` on future rate `R_j`. |
...
This notation separates three temporal roles. Baseline covariates `X(t_i0)` are
candidate causes, baseline targets `Y(t_i0)` provide autoregressive disease
state, and future rates `R` are the effects to be predicted and explained. This
separation is central: BN-LTE does not infer causality from simultaneous
correlation alone.

## Algorithm 1: BN-LTE Training and Evaluation Pipeline

```text
Input:
    Raw ADNI tables, participant identifiers, visit dates,
    baseline multimodal variables, regional target variables,
    biological graph constraints, spline specification,
    candidate regularization strengths, and train/validation/test seed.

Output:
    Fitted BN-LTE model, held-out follow-up predictions,
    dynamic edge curves, performance metrics, and visualization artifacts.

1. Construct longitudinal target pairs.
    a. For each participant, order valid visits by acquisition date.
    b. Select baseline-follow-up pairs with positive elapsed time.
    c. Compute Delta_i in years.
    d. Compute annualized rates R_ij = (Y_ij(t_i1) - Y_ij(t_i0)) / Delta_i.

2. Build the baseline design matrix.
    a. Assemble X_i(t_i0) from demographics, APOE, fluid biomarkers,
       amyloid-PET, tau-PET, MRI-derived neurodegeneration measures,
       and cognitive scores.
    b. Store baseline targets Y_i(t_i0), follow-up targets Y_i(t_i1),
       and rates R_i.
    c. Remove variables with insufficient training coverage or
       near-zero training variance.

3. Split by participant.
    a. Partition unique participants into training, validation, and test sets.
    b. Ensure that no participant contributes pairs to more than one split.

4. Estimate latent disease time Z.
    a. Select pseudotime features M_i from baseline variables only.
    b. In tau-free mode, exclude tau-PET and p-tau217 variables from M_i.
    c. Impute and standardize M_i using training statistics.
    d. Fit a one-dimensional training SVD embedding.
    e. Orient the embedding by an interpretable disease-burden proxy.
    f. Map projected scores to Z_i in [0,1] using training percentiles.

5. Define admissible graph structure.
    a. Assign variables to biological layers: exogenous roots,
       molecular pathology, imaging pathology, neurodegeneration,
       and clinical outcomes.
    b. Prohibit incoming edges into root variables.
    c. Prohibit outgoing edges from clinical outcome variables.
    d. Prohibit edges that violate the baseline-to-future temporal order.
    e. For each target j, rank admissible parents by coverage,
       biological priority, and training association.

6. Fit stage-varying structural equations.
    For each target j:
        a. Evaluate spline basis B(Z_i) on training rows.
        b. Construct Phi_j from stage intercept terms,
           baseline self-history terms, and admissible parent terms:
               B_k(Z_i),
               Y_ij(t_i0) B_k(Z_i),
               X_il(t_i0) B_k(Z_i).
        c. Fit penalized MAP/ridge coefficients on training rows.
        d. Select regularization by validation performance.
        e. Recover a_j(z), c_j(z), and b_jl(z) on a dense Z grid.

7. Predict held-out progression.
    a. For each validation or test pair, compute Z_i using the fitted
       training embedding.
    b. Predict future rates R_hat_ij from the fitted structural equation.
    c. Convert rates to follow-up burden:
           Y_hat_ij(t_i1) = Y_ij(t_i0) + Delta_i R_hat_ij.

8. Evaluate and summarize.
    a. Compute rate and follow-up-burden MAE/RMSE.
    b. Compute spatial rank agreement between empirical and predicted
       regional burden maps.
    c. Compare BN-LTE against NDM, ESM, SIR, persistence, and related models.
    d. Export dynamic edge curves, causal-order summaries, and brain-region
       visualizations comparing baseline, empirical follow-up, and model
       predictions.
```
...
Let subject-pair `i` contain a baseline visit at time `t_i0` and a follow-up
visit at time `t_i1`, with elapsed time `Delta_i > 0` measured in years. For
each regional tau-PET target `j`, we observe baseline burden `Y_ij(t_i0)` and
follow-up burden `Y_ij(t_i1)`. The learning target is the annualized
longitudinal rate
...
`Z_i in [0,1]` from baseline multimodal measurements. Conditional on `Z_i`, the
model defines a directed Bayesian-network structural causal model over future
rates. The key assumption is that the parent set and causal effect size of a
biomarker can be stage dependent: an edge that is weak early in disease can
...
age, sex, education, and APOE epsilon-4 dosage; fluid variables include plasma
p-tau217, A beta 42/40, NfL, and GFAP; pathology variables include amyloid-PET
burden and regional tau-PET SUVR; neurodegeneration variables include
FreeSurfer-derived hippocampal, amygdala, and temporal cortical measures; and
clinical variables include ADAS-Cog-13, MMSE, RAVLT immediate recall, and CDR-SB.
...
where each `R_ij` is computed from the subject-specific follow-up interval
`Delta_i`. In the model-comparison experiment, the target regions are bilateral
entorhinal, fusiform, inferior temporal, middle temporal, and inferior parietal
cortex. Subject-level train, validation, and test splits are made by unique
participant identifier so that no subject contributes pairs to more than one
split.

## Latent Time Embedding
...
is then applied unchanged to validation and test rows. This avoids using held-out
outcomes to define the disease axis.

Let `M_i subset X_i(t_i0)` be the subset of baseline features permitted for
...
MRI volume, cortical thickness, and A beta 42/40. Scores are mapped to the unit
interval using the first and ninety-ninth training percentiles:
...
decomposed into feature-level contributions
`\widetilde{M}_{im} w_m`, and population-level loadings identify which
biomarkers define the disease axis.

## Dynamic Bayesian-Network Factorization
...
Here `pa_z(j)` is the set of admissible baseline parents for target `j` at
latent time `z`. In the current implementation, a target has a fixed candidate
parent set selected before estimation, while the corresponding effect functions
can become effectively active or inactive across `z`. Thus, graph
non-stationarity is represented by smooth stage-varying effect curves rather
than by independently estimating a separate graph at every time point.
...
`c_j(Z_i)Y_ij(t_i0)` is a self-history term that captures autonomous
progression from existing local burden; and `b_jl(Z_i)` is the direct
stage-dependent effect of baseline parent `l` on the future rate of target `j`.
The coefficient curve `b_jl(z)` is interpreted as a conditional
...
## Spline Parameterization of Non-Stationary Effects
...
`B_1(z), ..., B_K(z)` be cubic B-spline basis functions on `[0,1]`. Then
...
including plasma p-tau217, amyloid-PET burden, A beta 42/40, APOE epsilon-4
dosage, and age. The final parent set is capped at a fixed maximum size to
control variance in the high-dimensional spline design.

## Estimation as Penalized Maximum A Posteriori Learning

For target `j`, let `r_j` be the vector of finite training rates and
`Phi_j` the corresponding design matrix. BN-LTE estimates spline coefficients
...
The regularization strength `lambda_j` is chosen by group-aware cross-validation
on the training set, where folds are grouped by subject identifier. This keeps
multiple longitudinal pairs from the same participant inside the same fold and
prevents subject-level leakage during hyperparameter selection.
...
## Edge Curves and Causal Summaries
...
`Y_i(t_i1)-Y_i(t_i0)` to test whether the model captures the spatial ordering
of progression beyond baseline persistence.

BN-LTE is compared against network diffusion, epidemic spreading, susceptible-
infected-recovered spreading, and baseline persistence. The graph-spreading
baselines are fit on the full 68-region tau state using the group-average
connectome, while BN-LTE is fit directly to selected regional annualized tau
rates. This comparison is intentionally conservative for absolute follow-up
SUVR, because baseline persistence already explains much of the spatial rank
structure; the more informative test for disease dynamics is the held-out rate
and delta-rank prediction.
...
associated with a higher subsequent tau accumulation rate in that target region.
